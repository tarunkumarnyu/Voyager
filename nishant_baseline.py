...
class KeyboardPlayerPyGame(Player):
    def __init__(self):
        ...
        self.visited_locations = []  # List of (index, frame_count) tuples
        self.loop_closures = []      # List of detected loop closures
        self.frame_count = 0         # Track frame number
        self.loop_closure_threshold = 50  # Minimum frames between visits to count as loop closure
    ...
    def print_loop_closure_summary(self):
        """Print summary of detected loop closures."""
        print(f"\n{'='*60}")
        print(f"LOOP CLOSURE SUMMARY")
        print(f"{'='*60}")
        print(f"Total loop closures detected: {len(self.loop_closures)}")
        print(f"Total locations visited: {len(self.visited_locations)}")
        
        if len(self.loop_closures) > 0:
            print(f"\nDetected Loop Closures:")
            for i, lc in enumerate(self.loop_closures, 1):
                print(f"  {i}. Location {lc['filename']} (ID: {lc['current_index']})")
                print(f"     First visit: frame {lc['previous_frame']}")
                print(f"     Return visit: frame {lc['current_frame']}")
                print(f"     Frame gap: {lc['frame_gap']}")
        else:
            print("\nNo loop closures detected yet.")
        print(f"{'='*60}\n")

    def check_loop_closure(self, current_index):
        """
        Check if current location matches a previously visited location (loop closure).
        Returns (is_loop_closure, previous_visit_frame) or (False, None)
        """
        if len(self.visited_locations) == 0:
            return False, None
        
        # Check if we've visited this location before
        for prev_index, prev_frame in self.visited_locations:
            if prev_index == current_index:
                # Check temporal distance (must be far enough to count as loop closure)
                frame_gap = self.frame_count - prev_frame
                if frame_gap >= self.loop_closure_threshold:
                    # Verify with SuperGlue to ensure it's actually the same place
                    if self.superglue_available and self.superpoint is not None:
                        current_img = self.fpv
                        prev_path = os.path.join(self.save_dir, self.image_filenames[prev_index])
                        prev_img = cv2.imread(prev_path)
                        
                        if prev_img is not None:
                            num_inliers, is_valid = self.verify_match_with_superglue(current_img, prev_img)
                            if is_valid:
                                return True, prev_frame
                    
                    # If SuperGlue not available, trust CosPlace match
                    return True, prev_frame
    ...
    def display_next_best_view(self):
        ...
        is_loop_closure, prev_frame = self.check_loop_closure(current_index)
        
        # Track visited location
        if len(self.visited_locations) == 0 or self.visited_locations[-1][0] != current_index:
            self.visited_locations.append((current_index, self.frame_count))
            # Keep only last 100 visited locations to avoid memory issues
            if len(self.visited_locations) > 100:
                self.visited_locations.pop(0)
        
        # Log loop closure if detected
        if is_loop_closure:
            loop_info = {
            ...
            logging.info(f"LOOP CLOSURE DETECTED! Location {current_filename} (ID: {current_index}) "
                        f"visited at frame {prev_frame}, returned at frame {self.frame_count} "
                        f"(gap: {self.frame_count - prev_frame} frames)")
            print(f"\n{'='*60}")
            print(f"LOOP CLOSURE DETECTED!")
            print(f"Location: {current_filename} (ID: {current_index})")
            print(f"Previously visited at frame: {prev_frame}")
            print(f"Current frame: {self.frame_count}")
            print(f"Frame gap: {self.frame_count - prev_frame}")
            print(f"{'='*60}\n")
        ...
        loop_status = " [LOOP CLOSURE!]" if is_loop_closure else ""
        print(f'Current View: {current_filename} (ID: {current_index}){loop_status} || '
    ...
    def see(self, fpv):
        ...
        if self._state:
            ...
            # If in navigation stage
                ...
                # Increment frame count for loop closure tracking
                self.frame_count += 1
                ...
                if keys[pygame.K_g]:
                    self.display_goal_image()
                # If 'l' key is pressed, show loop closure summary
                if keys[pygame.K_l]:
                    self.print_loop_closure_summary()
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(player.loop_closures) > 0:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            output_file = f"loop_closures_{timestamp}.json"
            with open(output_file, 'w') as f:
                json.dump(player.loop_closures, f, indent=2)
            print(f"\nLoop closures saved to: {output_file}")
            print(f"Total loop closures: {len(player.loop_closures)}")
